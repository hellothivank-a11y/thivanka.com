...
function setJobSheetMarketFilter(marketKey) {
  ...
  ['all', 'uk', 'aus'].forEach(k => {
    ...
    if (marketKey.toLowerCase() === k) {
      btn.className = "px-3 py-1.5 rounded-lg text-xs font-semibold bg-white text-slate-900 dark:bg-zinc-800 dark:text-zinc-100 border border-slate-200 dark:border-zinc-700 shadow-sm transition-all";
    } else {
      btn.className = "px-3 py-1.5 rounded-lg text-xs font-medium text-slate-500 hover:text-slate-900 dark:text-zinc-400 dark:hover:text-zinc-200 transition-all";
    }
  });
  renderJobSheetTable();
}
...
function performTabSwitch(tabName) {
  ...
  tabs.forEach(t => {
    ...
    if (btn) {
      if (t === tabName) {
        btn.className = "px-3.5 py-1.5 rounded-lg text-xs font-semibold bg-white text-slate-900 dark:bg-zinc-800 dark:text-zinc-100 border border-slate-200/80 dark:border-zinc-700 shadow-sm flex items-center gap-2 transition-all";
      } else {
        btn.className = "px-3.5 py-1.5 rounded-lg text-xs font-medium text-slate-500 hover:text-slate-900 dark:text-zinc-400 dark:hover:text-zinc-200 flex items-center gap-2 transition-all";
      }
    }
  });
...
function updateLivePayoutStrip(isModal = false) {
  ...
  if (!hasJobDetails) {
    if (totalEl) {
      totalEl.textContent = '--';
      totalEl.className = "text-base font-bold text-slate-400 dark:text-zinc-500 tracking-tight";
    }
    if (badgesContainer) {
      badgesContainer.innerHTML = `
        <span class="text-slate-400 dark:text-zinc-500 text-[11px] font-medium flex items-center gap-1.5">
          <i data-lucide="info" class="w-3.5 h-3.5 text-slate-400 dark:text-zinc-600"></i> Enter job details to calculate payout
        </span>
      `;
    }
    if (stripContainer) {
  ...
  if (totalEl) {
    totalEl.className = "text-base font-bold text-slate-900 dark:text-zinc-100 tracking-tight";
  }
  if (stripContainer) {
...
function switchExcelPreviewTab(mode) {
  ...
  const activeClasses = ['bg-white', 'text-slate-900', 'dark:bg-zinc-800', 'dark:text-zinc-100', 'border', 'border-slate-200', 'dark:border-zinc-700', 'shadow-sm', 'font-semibold'];
  const inactiveClasses = ['text-slate-500', 'hover:text-slate-900', 'dark:text-zinc-400', 'dark:hover:text-zinc-200', 'font-medium'];

  if (mode === 'night') {
    tabNight?.classList.remove(...inactiveClasses);
    tabNight?.classList.add(...activeClasses);

    tabAll?.classList.remove(...activeClasses);
    tabAll?.classList.add(...inactiveClasses);
  } else {
    tabAll?.classList.remove(...inactiveClasses);
    tabAll?.classList.add(...activeClasses);

    tabNight?.classList.remove(...activeClasses);
    tabNight?.classList.add(...inactiveClasses);
  }
